package com.ltz.aliwechatpaydemo.wechatpay;
...
public class WechatPayUtil {
     ...
     * 调起微信支付的方法,不需要在客户端签名
     **/
    public void payNotSign(Context context, String appid) {
        iwxapi = WXAPIFactory.createWXAPI(context,appid,false); //初始化微信api
    ...
    /**
     * 调起微信支付的方法,需要在客户端签名
     * key（lkfjsklkfjskdflkfjskdfkdkfsdlfsd）  key为商户平台设置的密钥key
     **/
    public void payAndSign(Context context, String appid,final String key) {
        iwxapi = WXAPIFactory.createWXAPI(context,appid,false); //初始化微信api
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            @Override
            public void run() {
